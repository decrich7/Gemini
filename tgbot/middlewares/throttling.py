...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def target_throttled(target: Union[types.Message, types.CallbackQuery],
        ...
        if throttled.exceeded_count == 2:
            await msg.answer('❕ Отправлять сообщения не так часто, пожалуйста :)')
            return
        elif throttled.exceeded_count == 4:
            await msg.reply('❌❌❌<strong>Мы заметили подозрительную активность</strong>❌❌❌\nПопробуйте отправлять сообщения реже')
            return
